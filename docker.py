...
def count_all():
        with os.popen("docker ps -aq|wc -l") as pipe:
...
def count_exited():
        with os.popen("docker ps -qaf 'status=exited'|wc -l") as pipe:
                print(pipe.readline().strip())
...
def md5sum(args):
        ...
        if not 'md5sum_values' in locals():
                md5sum_values = ""
        print(md5sum_values.split()[0])


def version(args):
        with os.popen("docker ps --format '{{.Image}}' --filter id=" + args.container) as pipe:
                docker_version = pipe.read().strip()
        pipe.close()
        if 'docker_version' not in locals():
                docker_version = ""
        print(docker_version.split()[0])
...
if __name__ == "__main__":
        ...
        if len(sys.argv) > 2:
                ...
                parser.add_argument("stat", help="container stat",
                                                        choices=["status", "uptime", "cpu", "mem", "disk", "netin", "netout", "md5sum", "version"])
                ...
                if args.stat == "status":
                        ...
                        net_sent(args)
                elif args.stat == "md5sum":
                        debug("calling md5sum for " + args.container)
                        md5sum(args)
                elif args.stat == "version":
                        debug("calling md5sum for " + args.container)
                        version(args)
        elif len(sys.argv) == 2:
                if sys.argv[1] == "count":
                        debug("calling count")
                        count_running()
                elif sys.argv[1] == "count_all":
                        debug("calling count_all")
                        count_all()
                elif sys.argv[1] == "count_exited":
                        debug("calling count_exited")
                        count_exited()
        else:
